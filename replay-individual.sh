...
# Modify these
LOGDEV=/dev/nvme1n1p2
REPLAYDEV=/dev/nvme0n1p1
FSCK=fsck.f2fs
TEST_MNT=/mnt/test
START_MARK="test"
...
dd if=/dev/zero of=cow-dev bs=1M count=100 #seek=1048576
...
do
	...
	$FSCK $TARGET | grep CKPT #> /dev/null 2>&1 || _fail "fsck failed at entry $ENTRY"
	mount $TARGET $TEST_MNT || _fail "mount failed at entry $ENTRY"
	umount $TEST_MNT
	$FSCK $TARGET | grep CKPT #> /dev/null 2>&1 || _fail "fsck failed after mount at " \
		#"entry $ENTRY"
	dmsetup --retry remove $SNAPSHOTCOW 2>/dev/null || _fail "failed to remove snapshot"
	dmsetup --retry remove $SNAPSHOTBASE 2>/dev/null || _fail "failed to remove base"
	let ENTRY+=1
done
