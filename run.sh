...
_do_aged_test()
{
	./postmark < postmark_run

	for i in `seq 1 100`
	do
		./create_bench $TESTDIR/m-$i $1 $2 $3 $4 >> aged_results
	done
}
...
_aged_test()
{
	...
	echo "===============================" >> aged_results
	echo "$DEV (old): 512k random updates on 1MB * 64 files = 64MB" >> aged_results
	./run.sh reload_ko f2fs_old
	_do_aged_test 128 64 1 0

	echo "===============================" >> aged_results
	echo "$DEV (old): 16k * 4k blocks * 1 file = 64MB" >> aged_results
	./run.sh reload_ko f2fs_old
	_do_aged_test $((16*1024)) 1 0 0

	echo "===============================" >> aged_results
	echo "$DEV: 512k random updates on 1MB * 64 files = 64MB" >> aged_results
	./run.sh reload_ko f2fs
	_do_aged_test 128 64 1 0

	echo "===============================" >> aged_results
	echo "$DEV: 16k * 4k blocks * 1 file = 64MB" >> aged_results
	./run.sh reload_ko f2fs
	_do_aged_test $((16*1024)) 1 0 0

	echo "===============================" >> aged_results
	echo "16k * 4k blocks * 1 file = 64MB" >> aged_results
	echo "$MAIN-$MAINDEV (noflush): 512k random updates on 1MB * 64 files = 64MB" >> aged_results
	./run.sh reload_mul fs/f2fs/f2fs
	_do_aged_test 128 64 1 0

	echo "===============================" >> aged_results
	echo "$MAIN-$MAINDEV (noflush): 16k * 4k blocks * 1 file = 64MB" >> aged_results
	./run.sh reload_mul fs/f2fs/f2fs
	_do_aged_test $((16*1024)) 1 0 0

	echo "===============================" >> aged_results
	echo "16k * 4k blocks * 1 file = 64MB" >> aged_results
	echo "$MAIN-$MAINDEV (flush): 512k random updates on 1MB * 64 files = 64MB" >> aged_results
	./run.sh reload_mul fs/f2fs/f2fs
	_do_aged_test 128 64 1 1

	echo "===============================" >> aged_results
	echo "$MAIN-$MAINDEV (flush): 16k * 4k blocks * 1 file = 64MB" >> aged_results
	./run.sh reload_mul fs/f2fs/f2fs
	_do_aged_test $((16*1024)) 1 0 1

	echo "===============================" >> aged_results
	echo "16k * 4k blocks * 1 file = 64MB" >> aged_results
	echo "$MAIN-$MAINDEV (copy-flush): 512k random updates on 1MB * 64 files = 64MB" >> aged_results
	./run.sh reload_mul f2fs_copy
	_do_aged_test 128 64 1 1

	echo "===============================" >> aged_results
	echo "$MAIN-$MAINDEV (copy-flush): 16k * 4k blocks * 1 file = 64MB" >> aged_results
	./run.sh reload_mul f2fs_copy
	_do_aged_test $((16*1024)) 1 0 1

	echo "===============================" >> aged_results
	echo "16k * 4k blocks * 1 file = 64MB" >> aged_results
	echo "$MAIN-$MAINDEV (copy-atomic-flush): 512k random updates on 1MB * 64 files = 64MB" >> aged_results
	./run.sh reload_mul f2fs_atomic
	_do_aged_test 128 64 1 1

	echo "===============================" >> aged_results
	echo "$MAIN-$MAINDEV (copy-atomic-flush): 16k * 4k blocks * 1 file = 64MB" >> aged_results
	./run.sh reload_mul f2fs_atomic
	_do_aged_test $((16*1024)) 1 0 1
}
...
_log_replay_start()
{
	TABLE="0 $(blockdev --getsz /dev/$LOG_TARGET) log-writes /dev/$LOG_TARGET /dev/$LOG_DEV"
	dmsetup create log --table "$TABLE"
	mkfs.f2fs -f /dev/mapper/log
	dmsetup message log 0 mark mkfs
	mount -t f2fs -o discard /dev/mapper/log /mnt/test
}
...
_log_replay_end()
{
	...
	echo ./replay-individual.sh
	echo ./replay-log --log /dev/nvme1n1p2 --find --end-mark test
	echo ./replay-log --log /dev/nvme1n1p2 --replay /dev/nvme0n1p1 --limit 3121
	echo ./replay-log --log /dev/nvme1n1p1 --replay /dev/nvme0n1p1 \ #--start-mark mkfs \
			--fsck "fsck.f2fs /dev/nvme0n1p1" --check fua --check flush

	echo "dmsetup message log 0 mark test in generic/tests/009"
}
...
reload_mul)
	...
	rmmod f2fs
	insmod ~/$VER/$2.ko
	mkfs.f2fs -f -O encrypt -c /dev/$MAINDEV /dev/$MAIN
